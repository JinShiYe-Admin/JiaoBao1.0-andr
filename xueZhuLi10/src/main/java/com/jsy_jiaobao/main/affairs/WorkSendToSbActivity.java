...
public class WorkSendToSbActivity extends BaseActivity implements PublicMethod, OnClickListener{
	...
	private TextView img_file;//当前单位名
	private LinearLayout layout_file;//
	private TextView img_photo;//当前单位名
	...
	ArrayList<String> selectFileList = new ArrayList<String>();
...
//		commMsgRevicerUnitList_str = mCache.getAsString(cache_commMsgRevicerUnitList);
//		if (!TextUtils.isEmpty(commMsgRevicerUnitList_str)){
...
//				String name = subUnitRevicer.get(i).getUintName();
//				Button btn = new Button(mContext);
//				btn.setText(name);
//				ArrayList<Object> tag = new ArrayList<Object>();
//				tag.add(0,subUnitRevicer.get(i));
//				tag.add(1,selit);
//				btn.setTag(tag);
//				btn.setOnClickListener(unitClickListener);
//				layout_lowerunit.addView(btn);
...
//			}
	...
	/**
	 * 生成接收人列表
	 * @param List<UserListItem> UserList
	 */
	@SuppressWarnings("unchecked")
	private void creatClassReciverView(GetUnitClassRevicer revicer,int RoleIdentity,String unitname){
		...
		for (int i = 0; i < viewNames.size(); i++) {
			ArrayList<Object> tag = (ArrayList<Object>) viewNames.get(i).getTag();
			if (unitname.equals((String)tag.get(0))) {
				haveView = true;
				viewNames.get(i).setVisibility(0);
			}else{
				viewNames.get(i).setVisibility(8);
		...
		if (!haveView) {
			...
			case 2://老师
				...
				if (null == selit2) {
					selit2 = new ArrayList<Selit>();
				}
				if (null == genselit2) {
					genselit2 = new ArrayList<Selit>();
				}
				if (null == stuselit2) {
					stuselit2 = new ArrayList<Selit>();
				}
			...
			case 3://家长
				...
				ArrayList<Selit> list1 = new ArrayList<Selit>();
			...
			case 4:
				...
				if (null == stuselit4) {
					stuselit4 = new ArrayList<Selit>();
				}
				if (null == selit4) {
					selit4 = new ArrayList<Selit>();
				}
				ArrayList<Selit> list4 = new ArrayList<Selit>();
