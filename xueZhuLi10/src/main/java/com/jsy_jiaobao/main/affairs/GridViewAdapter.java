...
public class GridViewAdapter extends BaseAdapter {
	...
	private List<Selit> checkedList = new ArrayList<Selit>();
	private List<Boolean> mChecked;
	/**短信直通车*/
	private List<SMSTreeUnitID> smsList = null;
	/**短信直通车*/
	private List<SMSTreeUnitID> checkedSMSList = new ArrayList<SMSTreeUnitID>();
	private int uType;
	 ...
	 * @param selitList
	 */
	public void setReciverLower(List<GroupUserList> selitList) {
        for(int i=0;i<selitList.size();i++){
            mChecked.add(false);
        }
	}
	/**
	 ...
	 * @return
	 */
	public Map<Integer,List<SMSTreeUnitID>> getCheckedSMS(){
		Map<Integer,List<SMSTreeUnitID>> checked = new HashMap<Integer, List<SMSTreeUnitID>>();
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		try {
			if (readerArray != null) {
				...
				if (mc == 1 || pc == 1) {
					name.setText(item.getString("TrueName").toString()+"(已查看)");
				}else{
					name.setText(item.getString("TrueName").toString());
				}
