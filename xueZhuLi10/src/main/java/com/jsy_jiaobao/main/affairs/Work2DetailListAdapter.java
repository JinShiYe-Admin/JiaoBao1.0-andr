...
public class Work2DetailListAdapter<T> extends BaseAdapter {
	...
	public Work2DetailListAdapter(Context mContext,
			OnClickListener onclickListener, Handler mHandler) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		// TextView tv_author =
		// viewHolder.getView(R.id.workdetails_tv_msgauthor);
		...
		if (getItemViewType(position) == WorkDetails) {// 事务详细内容
			...
			if (attList != null) {
				for (int i = 0; i < attList.size(); i++) {
					...
					if (names[names.length - 1].equals("jpg")
						...
						downLoadPic(tv, att, names[names.length - 1]);
			...
			if (WorkType == 1) {// 我发的
				...
				if (work.getReadFlag() < 2) {// 0未读，1已读未回复，2已回复
					reply.setVisibility(0);
					// reply.setText("待回复");
					// reply.setTextColor(mContext.getResources().getColor(R.color.red));
					reply.setBackgroundResource(R.drawable.icon_workdetails_uncomment);//待回复
				} else if (work.getReadFlag() == 2) {// 0未读，1已读未回复，2已回复
					reply.setVisibility(0);
					// reply.setText("再回复");
					// reply.setTextColor(mContext.getResources().getColor(R.color.black));
	...
	ArrayList<HttpHandler> httpHandlerList = new ArrayList<HttpHandler>();
	 ...
	 * @param names
	 */
	private void downLoadPic(final View view, Attlist att, String names) {
		...
		if (!file.exists()) {
			...
			if (!inList) {
				...
				// bitmapUtils.display(view, att.getDlurl());
