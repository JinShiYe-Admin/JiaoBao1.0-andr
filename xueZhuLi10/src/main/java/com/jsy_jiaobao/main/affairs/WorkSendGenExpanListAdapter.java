...
public class WorkSendGenExpanListAdapter extends BaseExpandableListAdapter {
	...
	public Object getChild(int groupPosition, int childPosition) {
		if (null != groupList) {
			return groupList.get(groupPosition).getUserList().getGenselit().get(childPosition);
		}
		return null;
	}
	/**
...
//	}
	...
	public ArrayList<Selit> getSelectSelit(){
		...
		for (int i = 0; i < mChecked.size(); i++) {
			...
			for (int j = 0; j < childList.size(); j++) {
				if (childList.get(j)) {
					try {
						list.add(groupList.get(i).getUserList().getGenselit().get(j));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	public int getChildrenCount(int groupPosition) {
		if (null != groupList && groupList.size()>0) {
			GenRevicer genrevicer = groupList.get(groupPosition).getUserList();
			if (genrevicer != null) {
				ArrayList<Selit> list = genrevicer.getGenselit();
				if (null != list) {
					int size = list.size();
		...
		if (null != groupList && groupList.size()>0) {
			...
			if (null != list) {
				...
				if (childPosition*2+1>=size) {
					...
					checkBox2.setText(selit2.getName().trim());
