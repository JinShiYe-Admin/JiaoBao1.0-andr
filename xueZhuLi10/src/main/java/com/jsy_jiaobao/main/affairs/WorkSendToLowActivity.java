...
public class WorkSendToLowActivity extends BaseActivity implements PublicMethod {
    @ViewInject(R.id.ui_sendmessage)
    ScrollView layout_ui;

    @ViewInject(R.id.sendmessage_layout_sendtoall)
    LinearLayout layout_sendtoall;
    @ViewInject(R.id.sendmessage_btn_lowerunit)
    Button btn_sendtolowerunit;
    @ViewInject(R.id.sendmessage_btn_lowerparent)
    Button btn_sendtolowerparent;
    @ViewInject(R.id.sendmessage_btn_lowerstudent)
    Button btn_sendtolowerstudent;
    @ViewInject(R.id.sendmessage_layout_content)
    LinearLayout layout_content;
    @ViewInject(R.id.sendmessage_layout_higher)
    HorizontalScrollView layout_higher;
    @ViewInject(R.id.sendmessage_layout_lower)
    HorizontalScrollView layout_lower;
    @ViewInject(R.id.sendmessage_layout_reciverlist)
    LinearLayout layout_reciver;
    @ViewInject(R.id.sendmessage_btn_currunit)
    Button btn_currunit;//当前单位
    @ViewInject(R.id.sendmessage_edt_content)
    IEditText edt_content;//内容框
    @ViewInject(R.id.sendmessage_checkBox_shotmsg)
    CheckBox cb_shotmsg;//是否是短信
    @ViewInject(R.id.sendmessage_btn_send)
    Button btn_send;//发布
    @ViewInject(R.id.sendmessage_tv_curr)
    TextView tv_curr;//当前单位名
    ...
    public void initViews() {
        ...
        layout_higher.setVisibility(View.GONE);
        layout_lower.setVisibility(View.GONE);
        btn_sendtolowerstudent.setVisibility(View.GONE);
        layout_sendtoall.setVisibility(View.VISIBLE);
    ...
    public void onEventMainThread(ArrayList<Object> list) {
        ...
        switch (tag) {
            ...
            case Constant.msgcenter_work_GetMsgAllReviceUnitList:
                ...
                if (MsgAll_ToGen) {
                    btn_sendtolowerparent.setVisibility(View.VISIBLE);
                    WorkSendMessageActivityController.getInstance().GetMsgAllRevicer_toSchoolGen();
                } else {
                    btn_sendtolowerparent.setVisibility(View.GONE);
                }
                if (MsgAll_ToSubUnitMem) {
                    btn_sendtolowerunit.setVisibility(View.VISIBLE);
                    WorkSendMessageActivityController.getInstance().GetMsgAllRevicer_toSubUnit();
                } else {
                    btn_sendtolowerunit.setVisibility(View.GONE);
                }
                break;
    ...
    private void initRevicerList(String cacheName, int flag) {
        if (flag == sub) {
            ...
            if (null != getUnitAdminRevicer) {
                List<UnitAdminRevicer> selitadmintomemList;
