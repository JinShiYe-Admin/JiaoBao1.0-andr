...
public class WorkMoreActivity extends BaseActivity implements IXListViewListener,PublicMethod{
	...
	@ViewInject(R.id.layout_xlistview)XListView listView;
	...
	private List<CommMsg> list = new ArrayList<CommMsg>();
	private List<FeeBack> list2 = new ArrayList<FeeBack>();
...
//							if ("more_2".equals(timeFlag)) {
//								ArrayList<FeeBack> a = GsonUtil.GsonToList(data, new TypeToken<ArrayList<FeeBack>>() {}.getType());
//								list2.addAll(a);
//								if (a.size()<20) {
//									havemore = false;
//								}else{
//									havemore = true;
//								}
//								listAdapter.setFBData(list2);
//								
//							}else{
								
								List<CommMsg> a = GsonUtil.GsonToList(data, new TypeToken<ArrayList<CommMsg>>() {}.getType());
...
//					listAdapter.setFromMe(fromme);
					...
					String str_time = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date);
