...
public class GenCheckWorkActivity extends BaseActivity implements
	...
	private Spinner spinner;// 下拉框选择学生
	private FrameLayout fLayout;//
	private CusListView listView;// 列表
	private SpinnerAdapter _Adapter;
	private ArrayList<GenInfo> genList = new ArrayList<GenInfo>();// 家长信息列表
	...
	private ArrayList<EduLevel> baseEduList = new ArrayList<EduLevel>();// 所有学历值列表
	private ArrayList<EduLevel> showEduList = new ArrayList<EduLevel>();// 显示的学力值列表
	...
	private int pageSize = 10;// 每页数据10
	private int isSelf = 1;// 是否查看状态
	...
	private RelativeLayout ly_screen;// 错题本筛选 控件
	private TextView tv_selectName;// 删选条件显示控件
	private String mSelect;// 筛选条件
	...
	protected void onActivityResult(int arg0, int arg1, Intent arg2) {
		...
		case 999:
			...
			// 筛选提交名称
			mSelect = (String) args.getString("Select");
			// 如果栓选条件内容部位空
			if (mSelect.length() > 0 || mSelect != null) {
				// 显示筛选条件
	...
	private ArrayList<StuHW> stuExList = new ArrayList<StuHW>();
	...
	public void onEventMainThread(ArrayList<Object> list) {
		...
		case Constants.WORKOL_GetStuHWList:
			...
			if (getStuHWList1.size() == 0 || getStuHWList1 == null) {
				// 如果学生作业未空，提示没有作业
				tv_nowork.setVisibility(View.VISIBLE);
				fLayout.setVisibility(View.GONE);
			} else {
